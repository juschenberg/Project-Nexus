...
const DEFAULT_SOURCE = '/static/data/Baeckerei_Workflow_Input.xlsx';
const API_URL = '';
const API_TOKEN = '';

if (apiMeta) {
  apiMeta.textContent = 'API: deaktiviert (Excel only)';
}
...
async function fetchPreferences() {
  if (!API_URL) {
    logStatus('API deaktiviert, überspringe Preferences.');
    return [];
  }
  ...
  plan.days.forEach(day => {
    ...
    if (!day.shifts.length) {
      ...
      blocks.forEach(block => {
        ...
        people.className = 'shift-people';
        people.textContent = block.assigned.length ? block.assigned.join(', ') : 'Noch niemand';
